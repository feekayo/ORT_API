...
class Model_collection extends CI_Model{
    ...
    public function get_playlist_id($name,$user_id){
    ...
    public function delete_collection($collection_id){
        ...
        $query2 = $this->db->query("DELETE FROM ort_subscribe WHERE collection_id='$collection_id'");
        if($query){
            return true;
        }else return false;
    }
...
**/    
    ...
    public function fetch_voices_by_id($post_id){
        
            $this->db->select('ort_posts.*');
            $this->db-> select('profiles.*');
            $this->db->from('ort_posts');
            $this->db->where('ort_posts.post_id',$post_id);   
            $this->db->join('profiles','ort_posts.user_id=profiles.user_id');
